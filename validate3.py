...
def recognize_speaker(wav_file, xeus_model, enrolled_embeddings_dir, threshold=10):
    ...
    # Ensure test embeddings are 1-D
    test_embs = test_embs.flatten() if len(test_embs.shape) > 1 else test_embs
    ...
    for emb in embeds:
        ...
        # Ensure enrolled embeddings are 1-D
        enroll_embs = enroll_embs.flatten() if len(enroll_embs.shape) > 1 else enroll_embs
    ...
    min_distance = distances[recognized_speaker]

    # Print recognized speaker and minimum distance
    print(f"Recognized speaker: {recognized_speaker}, Distance: {min_distance}")

    # Return recognized speaker and minimum distance regardless of the threshold
    return recognized_speaker, min_distance
...
def recognize_and_save_results(csv_file_path, xeus_checkpoint, enrolled_embeddings_dir):
    ...
    for idx, row in df.iterrows():
        ...
        for audio_file in audio_files:
            ...
            recognized_speaker, min_distance = recognize_speaker(audio_file, xeus_model, enrolled_embeddings_dir)
    ...
    result_csv_file_path = csv_file_path.replace('.csv', '_recognition_results66.csv')
    results_df.to_csv(result_csv_file_path, index=False)

    print(f"Recognition results saved at: {result_csv_file_path}")

# Example usage
csv_file_path = "D:\\Voice Authetication ESPNET\\updated_speaker_info33.csv"  # Replace with actual path to your CSV file
xeus_checkpoint = "D:\\Voice Authetication ESPNET\\XEUS\\model\\xeus_checkpoint.pth"  # Replace with actual XEUS model checkpoint
enrolled_embeddings_dir = "D:\\Voice Authetication ESPNET\\embeddings3"  # Replace with actual path to enrolled embeddings
