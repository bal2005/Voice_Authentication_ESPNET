...
# For ESPnet model zoo integration
...
def enroll(name, file):
    """Enroll a user with an audio file using ESPnet model from the model zoo
        inputs: str (Name of the person to be enrolled and registered)
                str (Path to the audio file of the person to enroll)
        outputs: None"""

    # Download and load the ESPnet model
    print("Loading ESPnet model....")
    try:
        d = ModelDownloader()
        model_info = d.download_and_unpack(
            "espnet/kan-bayashi/ljspeech_vits")  # Replace with desired model
        model = Speech2Text(**model_info)
    except Exception as e:
        print(f"Failed to load the ESPnet model. Error: {e}")
        exit()
    ...
    try:
        ...
        # Modify based on your embedding extraction needs
    ...
    try:
        # Save embeddings
        # Update with your path to save embeddings
        np.save(os.path.join('embeddings', speaker + ".npy"), enroll_embs)
